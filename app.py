...
# 用 session_state 快取：同一分頁內不重複下載；邏輯更新時自動重抓，避免舊資料污染結果。
if (
    "master_data" not in st.session_state
    or st.session_state.get("master_data_version") != DATA_CACHE_VERSION
):
    master_data = load_master_market_data(TICKER_POOL, PRELOAD_START, PRELOAD_END)
    st.session_state["master_data"] = master_data
    st.session_state["master_data_version"] = DATA_CACHE_VERSION
else:
...
if st.sidebar.button("🔄 重新下載最新數據"):
    ...
    st.session_state.pop("master_data_version", None)
    st.rerun()
